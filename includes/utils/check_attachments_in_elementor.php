...
function check_attachment_in_elementor($attachment_ids = [], $file_paths = [] ) {
    ...
    if (!empty($file_paths)) {
        $escaped_files = array_map(function($path) {
            ...
            return preg_quote($newPath, '/');
        }, $file_paths);

        // Patrón flexible para Elementor JSON (dobles backslashes)
        $elementor_data_regex = implode('|', array_map(function($path) {
            return str_replace('/', '\\\\/', $path) . '(?:-(?:[0-9]+x[0-9]+|[0-9\\.]+))?\\.[a-zA-Z]{2,5}';
        }, $escaped_files));

        // Patrón flexible para CSS y enclosure
        $elementor_css_regex = implode('|', array_map(function($path) {
            return $path . '(?:-(?:[0-9]+x[0-9]+|[0-9\\.]+))?\\.[a-zA-Z]{2,5}';
        }, $escaped_files));
    } else {
