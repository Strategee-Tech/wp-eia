...
SELECT 
    ...
    CASE 
        WHEN EXISTS (
            ...
            AND (
                (t.attachment_id IS NOT NULL AND pm.meta_key = '_thumbnail_id' AND pm.meta_value = t.attachment_id)
                OR (t.attachment_id IS NOT NULL AND pm.meta_key IN('_elementor_data') AND pm.meta_value REGEXP CONCAT('\"id\":', t.attachment_id))
                OR (t.path IS NOT NULL AND pm.meta_key IN('_elementor_data') AND pm.meta_value REGEXP REPLACE(t.path, '/', '\\\\\\\\/'))
                OR (t.path IS NOT NULL AND pm.meta_key IN('_elementor_css','enclosure') AND pm.meta_value REGEXP REPLACE(t.path, '/', '\\\\\\\\/'))
            )
        ) THEN 1
        ELSE 0
    END AS en_uso
...
return $results = $wpdb->get_results($sql, ARRAY_A);
