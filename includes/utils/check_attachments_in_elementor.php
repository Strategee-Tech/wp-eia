...
function check_attachment_in_elementor($attachment_ids = [], $file_paths = []) {
  ...
  // Precalcular basenames para búsqueda
    $basename_map = [];
    foreach ($file_paths as $path) {
        $basename_map[$path] = preg_quote(pathinfo($path, PATHINFO_FILENAME), '/');
    }

    // Inicializar usage map
    $usage_map = array_fill_keys($attachment_ids, false);

    $found_count  = 0;
    $total_needed = count($attachment_ids);
    ...
    foreach ($results as $row) {
        ...
        // Para elementor_data, elementor_css, enclosure
        foreach ($basename_map as $path => $basename_escaped) {
            if (preg_match('/' . $basename_escaped . '/', $meta_value)) {
                $attachment_id = get_attachment_id_by_path($path);
                if (isset($usage_map[$attachment_id])) {
                    $usage_map[$attachment_id] = true;
                    $found_count++;
                }
                break; // Si encontramos uno, seguimos con el siguiente resultado
            }
        }
    }
